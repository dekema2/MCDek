...
namespace MCDek.Gui.MapEditor
{
    ...
    sealed class Forester
    {
        ...
        public class ForesterArgs
        {
            ...
            public byte PLANTON = Block.grass;
            public Random rand;
            public Level inMap;
            public Level outMap;

            public byte bGroundSurface = Block.grass;
            public byte bTrunk = Block.trunk;
            public byte bFoliage = Block.leaf;
        ...
        class StickTree : Tree
        {
            public override void MakeTrunk()
            {
                ...
                {
                    args.outMap.SetTile(pos.x, pos.z, pos.y + i, args.bTrunk);
                }
            }
        }
                ...
                for (int y = start; y < end; y++)
                {
                    ...
                    for (int xoff = -rad; xoff < rad + 1; xoff++)
                    {
                        for (int zoff = -rad; zoff < rad + 1; zoff++)
                        {
                            ...
                            args.outMap.SetTile(pos[0] + xoff, pos[2] + zoff, y, args.bFoliage);
                        }
                    }
                }
            }
        ...
        class ProceduralTree : Tree
        {
            ...
            void CrossSection(Vector3i center, float radius, int diraxis, byte matidx)
            {
                ...
                for (int off1 = -rad; off1 <= rad; off1++)
                {
                    for (int off2 = -rad; off2 <= rad; off2++)
                    {
                        ...
                        coord[secidx1] = sec1;
                        coord[secidx2] = sec2;
                        args.outMap.SetTile(coord, matidx);
                    }
                }
            }
            ...
            public override void MakeFoliage()
            {
                ...
                foreach (Vector3i coord in foliageCoords)
                {
                    args.outMap.SetTile(coord, args.bFoliage);
                }
            }
            ...
            void MakeRoots(RootBase[] rootbases)
            {
                ...
                foreach (Vector3i coord in foliageCoords)
                {
                        ...
                        args.ROOTS == RootMode.Hanging)
                    {
                        ...
                        byte searchIndex = Block.air;
                        if (args.ROOTS == RootMode.ToStone)
                        {
                            searchIndex = Block.rock;
                        }
        ...
        public static int DistanceToBlock(Level map, Vector3f coord, Vector3f vec, byte blockType)
        {
            return DistanceToBlock(map, coord, vec, blockType, false);
        }

        public static int DistanceToBlock(Level map, Vector3f coord, Vector3f vec, byte blockType, bool invert)
        {
            coord += .5f;
            int iterations = 0;
            while (map.InBounds(new Vector3i(coord)))
            {
                byte blockAtPos = map.GetTile(new Vector3i(coord));
        ...
        void FindTrees(List<Tree> treelist)
        {
            ...
            {
                for (int z = 0; z < args.inMap.depth; z++)
                {
        ...
        Vector3i RandomTreeLoc(int height)
        {
            ...
            int mindim = Math.Min(args.inMap.width, args.inMap.depth);
            if (padding > mindim / 2.2)
            {
                padding = (int)(mindim / 2.2);
            }
            int x = args.rand.Next(padding, args.inMap.width - padding - 1);
            int z = args.rand.Next(padding, args.inMap.depth - padding - 1);
